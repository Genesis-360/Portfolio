...
const OptionWheel = ({
  ...
  const styleCacheRef = useRef(new WeakMap<HTMLDivElement, string>());
  ...
  const runFrame = useCallback((now: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Blur is quantized to whole pixels and redundant style writes are
      // skipped, so the browser can cache painted layers instead of
      // repainting every option on every frame.
      const blurPx = dist * cfg.blur;
      const transform = `translate(${x.toFixed(2)}px, calc(${y.toFixed(2)}px - 50%)) rotate(${rot.toFixed(3)}deg)`;
      const opacity = Math.max(cfg.minOpacity, 1 - dist * cfg.fade).toFixed(3);
      const filter =
        cfg.blur > 0 && Math.round(blurPx) > 0 ? `blur(${Math.round(blurPx)}px)` : 'none';
      const signature = `${transform}|${opacity}|${filter}`;
      if (styleCacheRef.current.get(el) === signature) continue;
      styleCacheRef.current.set(el, signature);
  ...
  // Sync config + callbacks after render (never during), then re-seat the
  // wheel so layout matches the fresh config.
  useEffect(() => {
    const remPx =
